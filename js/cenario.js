...
class Cenario {
    ...
    update() {
        ...
        for (let i = this.activeObstacles.length - 1; i > -1; i--) {
            ...
            if (this.activeObstacles[i].position.x < -150)
    ...
    static create(options, startX, groundY) {
        for (let i = 0; i < options.obstacles.length; i++) {
            ...
            if (options.obstacles[i].area == "ground")
                options.obstacles[i].position.y = groundY - options.obstacles[i].h;
            else if(options.obstacles[i].area == "sky")
                options.obstacles[i].position.y = options.obstacles[i].position.y + (Math.floor(Math.random() * 41) - 20);// gera uma valor aleatorio entre -20 e 20 
